document.addEventListener('DOMContentLoaded', function() {
    ...
    async function searchYouTubeVideo(searchTerm) {
        ...
        if (!searchTerm || typeof searchTerm !== 'string' || searchTerm.trim().length === 0) {
            console.log('searchYouTubeVideo: Invalid or empty search term:', searchTerm);
            return null;
        }
        ...
        try {
            ...
            if (trimmedTerm.includes('youtube.com/watch?v=') || trimmedTerm.includes('youtu.be/')) {
                console.log('Search term is a YouTube URL, extracting video info...');
                const response = await fetch(`/youtube/search?q=${encodeURIComponent(trimmedTerm)}`);
                ...
                console.log('Search term appears to be a YouTube video ID:', trimmedTerm);
                
                // Create a YouTube URL from the ID and fetch oEmbed data
                const youtubeUrl = `https://www.youtube.com/watch?v=${trimmedTerm}`;
                const response = await fetch(`/youtube/search?q=${encodeURIComponent(youtubeUrl)}`);
                ...
                console.log(`YouTube video info retrieved via oEmbed for video ID: ${data.title}`);
                return data;
            } else {
        ...
        try {
            ...
            pool.subscribeMany(relays, [{
                ...
                onevent(event) {
                    ...
                    displayNoteContent(event);
                     ...
                     setTimeout(() => {
                         console.log('Note loaded, checking if we should start playback. Queue length:', songQueue.length);
            ...
            setTimeout(() => {
                if (!document.getElementById('noteContent').textContent) {
                    ...
                    document.getElementById('noteLoaderContainer').style.display = 'block';
    ...
    function displayNoteContent(event) {
        ...
        const authorNameProfileImg = document.getElementById('authorNameProfileImg');
        
        // Process content for both images and nostr mentions
        processNoteContent(event.content).then(processedContent => {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (authorName.textContent === 'Author') {
                    ...
                    authorName.textContent = npub.substring(0, 8) + '...' + npub.substring(npub.length - 8);
                }
            }, 5000);
    ...
    function updateAuthorInfo(profileEvent) {
        try {
            ...
            // Update author name
            let displayName = authorContent.displayName || authorContent.display_name || authorContent.name;
            if (displayName) {
            ...
            console.log('Updated author info:', { name: displayName, picture: authorContent.picture });
        ...
        pool.subscribeMany(relays, [{
            ...
            onevent(event) {
                ...
                processZapEvent(event, initialLoadingComplete);
                 ...
                 setTimeout(() => {
                     console.log('Zaps EOS - checking if we should start playback. Queue length:', songQueue.length, 'isPlaying:', isPlaying, 'currentlyPlaying:', currentlyPlaying);
    ...
    function addToZapsList(zapInfo) {
        ...
        if (zapInfo.songInfo && zapInfo.songInfo.type === 'youtube') {
            ...
            const existingZap = json9735List.find(zap => 
    ...
    async function addToQueue(zapData) {
        ...
        console.log(`Added YouTube song to queue. Queue length: ${songQueue.length}, isPlaying: ${isPlaying}, currentlyPlaying: ${currentlyPlaying ? 'yes' : 'no'}`);
    ...
    async function addToQueueSilently(zapData) {
        ...
        console.log(`Silently added YouTube song to queue. Queue length: ${songQueue.length}`);
        console.log(`Song info:`, songInfo);
    ...
    function playNextSong() {
        console.log(`playNextSong called. Queue length: ${songQueue.length}, isPlaying: ${isPlaying}, currentlyPlaying: ${currentlyPlaying ? 'yes' : 'no'}`);
    ...
    function updateQueueDisplay() {
        ...
        songQueueElement.innerHTML = songQueue.map((item, index) => `
    ...
    function updatePlayedSongsDisplay() {
        ...
        playedSongsElement.innerHTML = playedSongs.map((item, index) => `
    ...
    async function createkinds9735JSON(kind9735List, kind0fromkind9735List){
        for(let kind9735 of kind9735List){
         ...
         console.log('Processing', json9735List.length, 'existing zaps for jukebox queue...');
    ...
    document.getElementById('note1LoaderSubmit').addEventListener('click', function() {
    ...
    async function processTopZap() {
        ...
        const validZaps = zapQueue.filter(zap => 
            zap.content && 
