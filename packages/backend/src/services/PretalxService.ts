...
export class PretalxService {
    ...
    private async apiGet<T>(path: string): Promise<T> {
        ...
        if (!res.ok) {
            ...
            this.logger.error(`Pretalx API error ${res.status} for ${url}: ${body.substring(0, 500)}`);
            throw new Error(`Pretalx API error ${res.status}: ${body.substring(0, 200)}`);
        }
        ...
        if (Array.isArray(data)) {
            ...
            this.logger.debug(`API returned paginated response with ${results.length} results`);
        }
        return data;
    }
    ...
    private async paginate<T>(path: string, key: string = 'results'): Promise<T[]> {
        ...
        while (urlPath) {
            ...
            if (Array.isArray(page)) {
                this.logger.debug(`Page ${pageNum}: got array with ${page.length} items`);
                (page as T[]).forEach(x => results.push(x));
                break;
            }
            const items = (page as Record<string, unknown>)[key] as T[] | undefined;
            if (items?.length) {
                this.logger.debug(`Page ${pageNum}: got ${items.length} items in '${key}' field`);
        ...
        this.logger.info(`Paginated fetch complete: ${results.length} total items across ${pageNum} page(s)`);
        return results;
    }

    public async fetchSchedulesList(): Promise<Array<{ id?: number | string; version?: string; published?: string | null }>> {
        ...
        return results.map(r => ({ id: r['id'] as number | string | undefined, version: r['version'] as string | undefined, published: (r['published'] as string | null | undefined) ?? null }));
    }
    ...
    private async fetchExpandedSlots(): Promise<PretalxExpandedSlot[]> {
        ...
        try {
            ...
            this.logger.info(`Successfully fetched ${slots.length} slots from Pretalx API`);
            if (slots.length > 0) {
                // Log first slot structure for debugging
                this.logger.debug(`First slot sample: ${JSON.stringify({
            ...
            this.logger.error(`Failed to fetch Pretalx slots: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    ...
    private async fetchScheduleLatestSlots(): Promise<PretalxScheduleSlot[]> {
        ...
        const expandQuery = encodeURI('?schedule_version=latest&expand=submission.speakers,submission.speakers.answers');
    ...
    public async fetchSlotsByVersion(version: string): Promise<PretalxScheduleSlot[]> {
        // Filter slots by schedule_version per Pretalx API
        const query = encodeURI(`?schedule_version=${version}&expand=room,submission.speakers,submission.speakers.answers`);
        const path = `/api/events/${this.eventSlug}/slots/${query}`;
        this.logger.info(`Fetching Pretalx slots for version ${version} via slots list: ${path}`);
        const slots = await this.paginate<PretalxScheduleSlot>(path);
        this.logger.info(`Fetched ${slots.length} slots for version ${version}`);
        return slots;
    }

    public async fetchScheduleExpanded(idOrShortcut: string): Promise<PretalxScheduleExpanded> {
        // idOrShortcut supports numeric id, 'latest', or 'wip'
        const query = encodeURI('?expand=slots,slots.room,slots.submission.speakers');
        const path = `/api/events/${this.eventSlug}/schedules/${idOrShortcut}/${query}`;
        this.logger.info(`Fetching Pretalx schedule ${idOrShortcut} expanded from: ${path}`);
    ...
    private async fetchSubmissions(): Promise<PretalxSubmission[]> {
        ...
        const query = encodeURI('?state=confirmed&expand=speakers,speakers.answers,slot');
    ...
    public async buildSlotsFromPretalx(): Promise<Array<{ startAt: string; endAt: string; items: Array<{ ref: string }> }>> {
        ...
        const aggregate = (
            records: Array<{ start?: string; end?: string; submission?: { speakers?: Array<PretalxSpeaker> } }>
        ) => {
            const slotsOut: Array<{ startAt: string; endAt: string; items: Array<{ ref: string }> }> = [];
            let skippedNoTime = 0;
            let skippedNoSubmission = 0;
            for (const rec of records) {
                if (!rec.start || !rec.end) { skippedNoTime++; continue; }
                const noteSet = new Set<string>();
                const speakers = rec.submission?.speakers || [];
                if (!rec.submission) { skippedNoSubmission++; }
                for (const sp of speakers) {
                    const note = this.extractNoteFromSpeaker(sp);
                    if (note) noteSet.add(note);
                }
                slotsOut.push({ startAt: rec.start, endAt: rec.end, items: Array.from(noteSet).map(ref => ({ ref })) });
            }
        ...
        try {
            ...
            this.logger.warn(`Schedules by-version fetch failed: ${e instanceof Error ? e.message : String(e)}`);
        }

        if (base.length > 0) {
            const { slotsOut, skippedNoTime, skippedNoSubmission } = aggregate(base);
            this.logger.info(`Processed slots (slots endpoints): ${slotsOut.length} valid, ${skippedNoTime} missing times, ${skippedNoSubmission} missing submissions`);
            slotsOut.sort((a, b) => new Date(a.startAt).getTime() - new Date(b.startAt).getTime());
            if (slotsOut.length > 0) return slotsOut;
        }

        // Attempt 2: /slots/ (expanded) — only if schedules produced nothing usable
        this.logger.warn('No usable slots from schedules. Falling back to /slots/ endpoint.');
        const expandedSlots = await this.fetchExpandedSlots();
        const { slotsOut: slotsFromSlots, skippedNoTime: sNoTime, skippedNoSubmission: sNoSub } = aggregate(expandedSlots);
        this.logger.info(`Processed /slots/ fallback: ${slotsFromSlots.length} valid, ${sNoTime} missing times, ${sNoSub} missing submissions`);
        slotsFromSlots.sort((a, b) => new Date(a.startAt).getTime() - new Date(b.startAt).getTime());
        return slotsFromSlots;
    }

    public async buildSlotsFromVersion(version: string): Promise<Array<{ startAt: string; endAt: string; items: Array<{ ref: string }> }>> {
        ...
        if (version === 'latest' || version === 'wip') {
            try {
                ...
                this.logger.warn(`Expanded schedule fetch failed for ${version}, falling back to slots list filter: ${e instanceof Error ? e.message : String(e)}`);
        ...
        this.logger.info(`Version ${version} processed: ${slotsOut.length} valid, ${skippedNoTime} missing times`);
        slotsOut.sort((a, b) => new Date(a.startAt).getTime() - new Date(b.startAt).getTime());
        return slotsOut;
    }
    ...
    public async buildPreview(version: string, roomId?: number | string): Promise<Array<{ startAt: string; endAt: string; items: Array<{ ref: string }>; room?: { id?: number | string; name?: unknown } }>> {
        ...
        if (version === 'latest' || version === 'wip') {
            try {
                const schedule = await this.fetchScheduleExpanded(version);
                records = (schedule.slots || []) as PretalxScheduleSlot[];
                this.logger.info(`Fetched ${records.length} slots from expanded schedule ${version}`);
            } catch (e) {
                this.logger.warn(`Expanded schedule fetch failed for ${version}, falling back to slots filter: ${e instanceof Error ? e.message : String(e)}`);
                records = await this.fetchSlotsByVersion(version);
            }
        } else {
            records = await this.fetchSlotsByVersion(version);
        ...
        for (const rec of records) {
            ...
            if (typeof rec.room === 'object' && rec.room !== null) {
                ...
                if (typeof roomObj.name === 'string') {
                    ...
                    nameStr = (ml['en'] as string) || (ml[Object.keys(ml)[0]] as string) || String(roomObj.name);
                }
