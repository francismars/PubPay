...
interface NoteData {
  ...
  zappers: Map<
    string,
    { pubkey: string; amount: number; profile: Kind0Event | null }
  >;
}
...
export const MultiStatsPage: React.FC = () => {
  ...
  const loadRoomData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${getApiBase()}/multi/${roomId}/display`);
  ...
  const calculateStats = useCallback(
    async (room: RoomState): Promise<Stats> => {
      if (!nostrClientRef.current) {
      ...
      refToEventId.forEach((eventId, ref) => {
        ...
        const authorProfile = note
          ? authorProfiles.get(note.pubkey) || null
          : null;

        // Process zaps for this event
        const { totalZapAmount, zappers, zapDetails } = processZaps(
          zapEvents,
          zapPayerProfiles
        );
  ...
  const updateStats = useCallback(
    async (force = false) => {
      ...
      try {
        ...
        eventIdsRef.current = newStats.notes
          .map(n => n.eventId)
          .filter(Boolean);
      } catch (err) {
        setError(
  ...
  const subscribeToUpdates = useCallback(() => {
    ...
    const eventIds =
      eventIdsRef.current.length > 0
        ? eventIdsRef.current
        : stats?.notes.map(n => n.eventId).filter(Boolean) || [];
      ...
      >
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 14, color: '#6b7280' }}>
            ...
            {isLoading && (
              <span style={{ marginLeft: 8, color: '#4a75ff' }}>
                Updating...
              </span>
            )}
          </div>
        </div>
        <div style={{ fontSize: 12, color: '#6b7280' }}>Room ID: {roomId}</div>
      </div>
          ...
          <>
            ...
            >
              ...
              >
                ...
                <div
                  style={{ fontSize: 32, fontWeight: 700, color: '#4a75ff' }}
                >
            ...
            {stats.topPerformingLives.length > 0 && (
              ...
              >
                ...
                >
                  {stats.topPerformingLives
                    .slice(0, topLivesToShow)
                    .map((livePerf, idx) => {
                        ...
                        ? (() => {
                            try {
                              const profileData = JSON.parse(
                                noteData.authorProfile.content || '{}'
                              );
                              return (
                                sanitizeImageUrl(profileData.picture) || null
                              );
                            } catch {
                              return null;
                            }
                            ...
                            >
                              ...
                              >
                                {noteData.note && noteData.authorProfile
                                  ? getDisplayName(
                                      noteData.authorProfile,
                                      noteData.note.pubkey
                                    )
                                  : `Live #${rank}`}
                              </div>
                            </div>
                            <div style={{ fontSize: 12, color: '#6b7280' }}>
                              {noteData.zapCount} zap
                              {noteData.zapCount !== 1 ? 's' : ''} •{' '}
                              {noteData.zappers.size} zapper
                              {noteData.zappers.size !== 1 ? 's' : ''}
                            </div>
                            {noteData.note && (
                              <div
                                style={{
                ...
                {stats.topPerformingLives.length > topLivesToShow && (
                  <div style={{ marginTop: 16, textAlign: 'center' }}>
                    <button
                      onClick={() =>
                        setTopLivesToShow(prev =>
                          Math.min(prev + 10, stats.topPerformingLives.length)
                        )
                      }
                      style={{
                        padding: '10px 24px',
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    Sum of All Individual Zaps
                  </div>
                  <div
                    style={{ fontSize: 24, fontWeight: 700, color: '#111827' }}
                  >
                    {formatSats(
                      stats.notes.reduce((sum, n) => sum + n.zapAmount, 0)
                    )}
                  </div>
                ...
                >
                  ...
                  >
                    {formatSats(
                      stats.notes.reduce(
                        (sum, n) =>
                          sum + n.zapDetails.reduce((s, z) => s + z.amount, 0),
                        0
                      )
                    )}
                  </div>
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 24, fontWeight: 700, color: '#4a75ff' }}
                  >
                    {formatSats(stats.totalZapAmount)}
                  </div>
                  <div style={{ fontSize: 12, color: '#9ca3af' }}>sats</div>
                    ...
                    style={{
                      ...
                      color:
                        stats.notes.reduce(
                          (sum, n) =>
                            sum +
                            n.zapDetails.reduce((s, z) => s + z.amount, 0),
                          0
                        ) === stats.totalZapAmount
                          ? '#10b981'
                          : '#ef4444'
                    }}
                  >
              ...
              <div
                style={{
                  padding: 12,
                  backgroundColor: stats.notes.every(
                    n =>
                      n.zapDetails.reduce((s, z) => s + z.amount, 0) ===
                      n.zapAmount
                  )
              ...
              >
                <strong>Per-Live Verification:</strong>{' '}
                {
                  stats.notes.filter(
                    n =>
                      n.zapDetails.reduce((s, z) => s + z.amount, 0) ===
                ...
                of {stats.notes.length} lives verified
                {stats.notes.some(
                  n =>
                    n.zapDetails.reduce((s, z) => s + z.amount, 0) !==
                    n.zapAmount
                  ...
                  <div style={{ marginTop: 4, fontSize: 12 }}>
                    Some lives have accounting mismatches. Click on individual
                    lives to see details.
                  </div>
                )}
              </div>
            </div>
                          ...
                          <>
                            <div
                              style={{
                                fontSize: 11,
                                color:
                                  noteData.note.id === noteData.eventId
                                    ? '#10b981'
                                    : '#ef4444',
                                marginBottom: 4
                              }}
                            >
                              Note ID: {noteData.note.id.substring(0, 16)}...{' '}
                              {noteData.note.id === noteData.eventId
                                ? '✓'
                                : '✗ MISMATCH!'}
                            </div>
                          ...
                          {Array.from(noteData.zappers.values())
                            ...
                            .map(zapper => (
                              ...
                              >
                                ...
                                <span
                                  style={{ color: '#4a75ff', fontWeight: 600 }}
                                >
                  ...
                  >
                    ...
                    >
                      {(() => {
                        try {
                          const profileData = JSON.parse(
                            selectedLive.authorProfile.content || '{}'
                          );
                          if (profileData.picture) {
                            const sanitized = sanitizeImageUrl(
                              profileData.picture
              ...
              <div style={{ marginBottom: 32 }}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      {formatSats(
                        selectedLive.zapDetails.reduce(
                          (sum, z) => sum + z.amount,
                          0
                        )
                      )}
