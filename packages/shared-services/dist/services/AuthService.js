...
export class AuthService {
    ...
    static async deriveDeviceEncryptionKey() {
        ...
        const keyMaterial = await crypto.subtle.importKey('raw', new TextEncoder().encode(deviceKey), { name: 'PBKDF2' }, false, ['deriveKey']);
     ...
     */
    static async encryptWithDeviceKey(data) {
        const encryptionKey = await this.deriveDeviceEncryptionKey();
        // Generate random IV
        const iv = crypto.getRandomValues(new Uint8Array(12));
        // Encrypt
        const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, encryptionKey, new TextEncoder().encode(data));
        // Convert to base64
     ...
     */
    static async storeAuthData(publicKey, privateKey, method, password) {
    ...
    static getStoredAuthData() {
        ...
        if (privateKeyStr) {
            try {
                ...
                encryptedPrivateKey = JSON.parse(privateKeyStr);
            }
            catch {
     ...
     */
    static async decryptStoredPrivateKey(password) {
     ...
     */
    static async getCurrentUserPrivateKey(password) {
...
AuthService.decryptedKeyCache = null;
