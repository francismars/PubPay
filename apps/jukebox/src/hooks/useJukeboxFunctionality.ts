...
export const useJukeboxFunctionality = () => {
  ...
  const profileCacheRef = useRef<Map<string, {name: string, picture: string}>>(new Map());
  ...
  const loadNoteAndStartJukebox = async (noteId: string) => {
    try {
      ...
      noteSubscriptionRef.current = poolRef.current.subscribe(
        ...
        },
        {
          onevent(event: any) {
            ...
            setTimeout(() => {
              console.log('Note loaded, checking if we should start playback. Queue length:', songQueueRef.current.length);
              if (songQueueRef.current.length > 0 && !currentlyPlayingRef.current) {
      ...
      setError('Error loading note: ' + (error instanceof Error ? error.message : 'Unknown error'));
      setIsLoading(false);
      currentNoteIdRef.current = null;
    }
  };
        ...
        },
        {
          onevent(profileEvent: any) {
            ...
            try {
              ...
              profileCacheRef.current.set(authorPubkey, {name: displayName, picture: picture});
            } catch (error) {
  ...
  const subscribeToZaps = (eventId: string) => {
    try {
      ...
      if (allZapsLoadedRef.current || zapsSubscriptionRef.current) {
        console.log('Zaps already loaded or subscription active, skipping duplicate subscription');
        return;
      }
      ...
      zapsSubscriptionRef.current = poolRef.current.subscribe(
        ...
        },
        {
          ...
          oneose() {
            ...
            setTimeout(() => {
              console.log('Zaps EOS - checking if we should start playback. Queue length:', songQueueRef.current.length, 'isPlaying:', isPlayingRef.current, 'currentlyPlaying:', currentlyPlayingRef.current);
              
              // Only start playback if nothing is currently playing
              if (songQueueRef.current.length > 0 && !currentlyPlayingRef.current) {
                console.log('Starting playback after zaps loaded - no song currently playing');
                playNextSong();
              } else if (songQueueRef.current.length > 0 && currentlyPlayingRef.current && !isPlayingRef.current) {
                console.log('Resuming playback after zaps loaded - song exists but not playing');
  ...
  const processZapEvent = async (event: any, initialLoadingComplete: boolean = false) => {
    try {
      ...
      // Parse the zap event
      const description = event.tags.find((tag: any) => tag[0] === 'description');
      ...
      try {
        ...
        if (processedProfilesRef.current.has(zapData.pubkey)) {
          ...
          const cachedPicture = cachedProfile ? cachedProfile.picture : '/images/gradient_color.gif';
        ...
        if (activeSubscriptionsRef.current.has(zapData.pubkey)) {
          console.log('Profile subscription already active for:', zapData.pubkey);
        ...
        poolRef.current.subscribe(
          ...
          },
          {
            async onevent(profileEvent: any) {
              ...
              // Store profile data in cache
              profileCacheRef.current.set(zapData.pubkey, {name: zapperName, picture: zapperPicture});
              ...
              console.log(`Processing zap event: amount=${amount}, content="${zapData.content || ''}", zapper=${zapperName}`);
        ...
        setTimeout(async () => {
          if (zapperName === 'Unknown') {
            console.log('Profile lookup timeout, using fallback for zapper:', zapperPubkey);
            ...
            if (songInfo && songInfo.type === 'youtube') {
              ...
              console.log('Skipping zap timeout fallback - no valid YouTube content:', songInfo);
            }
          }
        }, 3000);
        ...
        if (songInfo && songInfo.type === 'youtube') {
          ...
          console.log('Skipping zap error fallback - no valid YouTube content:', songInfo);
        }
      }
  ...
  const extractYouTubeInfo = async (zapContent: string) => {
    ...
    const youtubeUrlRegex = /(https?:\/\/(?:www\.)?youtube\.com\/watch\?v=([\w\-]+)(?:[&\w=]*)|https?:\/\/youtu\.be\/([\w\-]+)(?:\?[&\w=]*)?)/gi;
    ...
    if (cleanContent && 
        ...
        !/^[^\w\s]*$/.test(cleanContent)) { // Not just special characters
        
      console.log(`extractYouTubeInfo: Found valid search term: "${cleanContent}", creating search result`);
      ...
      console.log(`extractYouTubeInfo: Content validation failed:`, {
        ...
        isJustSpecialChars: cleanContent ? /^[^\w\s]*$/.test(cleanContent) : false
      });
    }
  ...
  const addToQueue = async (zapData: any) => {
    ...
    console.log(`Added YouTube song to queue. Queue length: ${songQueueRef.current.length}, isPlaying: ${isPlayingRef.current}, currentlyPlaying: ${currentlyPlayingRef.current ? 'yes' : 'no'}`);
    console.log(`Song info:`, songInfo);
    
    // Always try to start playback if we have songs and nothing is currently playing
    if (!currentlyPlayingRef.current) {
      console.log('No song currently playing, starting playback immediately...');
      playNextSong();
  ...
  const addToQueueSilently = async (zapData: any) => {
    const songInfo = await extractYouTubeInfo(zapData.kind9735content);
    if (!songInfo || songInfo.type !== 'youtube') {
      console.log('Skipping zap silently - no valid YouTube content:', songInfo);
      return;
    }
    ...
    console.log(`Silently added YouTube song to queue. Queue length: ${songQueueRef.current.length}`);
    console.log(`Song info:`, songInfo);
  ...
  const addToZapsList = (zapInfo: any) => {
    ...
    if (zapInfo.songInfo && zapInfo.songInfo.type === 'youtube') {
      ...
      const existingZap = json9735ListRef.current.find(zap => 
      ...
      console.log('Skipping zap display - no valid YouTube content:', zapInfo.songInfo);
    }
  };
  ...
  const drawKinds9735 = (json9735List: any[]) => {
    ...
    console.log(`Filtered ${json9735List.length} total zaps to ${youtubeZaps.length} YouTube zaps`);
  ...
  const playNextSong = () => {
    console.log(`playNextSong called. Queue length: ${songQueueRef.current.length}, isPlaying: ${isPlayingRef.current}, currentlyPlaying: ${currentlyPlayingRef.current ? 'yes' : 'no'}`);
    ...
    console.log(`Playing next song: ${nextSong.songInfo.originalContent}, amount: ${nextSong.amount} sats`);
  ...
  const updateCurrentVideoDisplay = () => {
    ...
    if (!currentlyPlayingRef.current) {
      ...
      if (songQueueRef.current.length > 0) {
        console.log('Songs in queue but nothing playing, showing queue status...');
  ...
  const updateQueueDisplay = () => {
    ...
    if (songQueueRef.current.length === 0) {
      songQueueElement.innerHTML = '<div class="empty-queue">🎵 No songs in queue</div>';
      return;
    }
    
    songQueueElement.innerHTML = songQueueRef.current.map((item, index) => `
  ...
  const updatePlayedSongsDisplay = () => {
    ...
    if (playedSongsRef.current.length === 0) {
      playedSongsElement.innerHTML = '<div class="empty-history">📚 No songs played yet</div>';
      return;
    }
    
    playedSongsElement.innerHTML = playedSongsRef.current.map((item, index) => `
  ...
  const updateStats = () => {
    ...
    if (queueCountElement) queueCountElement.textContent = songQueueRef.current.length.toString();
    if (playedCountElement) playedCountElement.textContent = playedSongsRef.current.length.toString();
    
    const queueTotal = songQueueRef.current.reduce((sum, item) => sum + item.amount, 0);
    const playedTotal = playedSongsRef.current.reduce((sum, item) => sum + item.amount, 0);
  ...
  const processTopZap = async () => {
    ...
    const validZaps = zapQueueRef.current.filter(zap => 
      zap.content && 
    ...
    if (currentTopZapRef.current && currentTopZapRef.current.zapId === topZap.zapId) {
    ...
    try {
      ...
      if (!topZap.content || typeof topZap.content !== 'string' || topZap.content.trim().length === 0) {
  ...
  const handleJukeboxSubmit = () => {
    const noteInput = document.getElementById('note1LoaderInput') as HTMLInputElement;
