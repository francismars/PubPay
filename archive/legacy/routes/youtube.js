...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    if (!videoId) {
      ...
      console.log(
        'Search term provided, but oEmbed only works with direct video URLs'
      );
      return res.json({
        type: 'search',
        searchTerm: q,
        message:
          'Please provide a direct YouTube URL for automatic info extraction'
      });
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `oEmbed responded with status: ${response.status}: ${errorText}`
      );
    }
