...
export interface UseNostrSubscriptionsOptions {
  ...
  onLiveEventZap?: (zap: Kind9735Event, pubkey: string, identifier: string) => void;
...
export function useNostrSubscriptions(
        ...
        onclosed: () => {
          ...
          if (
            ...
            setTimeout(() => {
              ...
              if (eventInfo) {
                subscribeLiveEvent(eventInfo.pubkey, eventInfo.identifier, eventInfo.kind);
              }
  ...
  const subscribeLiveEventZaps = useCallback(async (pubkey: string, identifier: string) => {
    ...
    return subscription;
  }, [nostrClient, liveEventService, onLiveEventZap, resetZapList, markInitialZapsLoaded]);
  ...
  const subscribeKind9735fromKind1 = useCallback(async (kind1: Kind1Event) => {
    ...
    return subscription;
  }, [nostrClient, markInitialZapsLoaded, onZapsLoaded, subscribeKind0fromKinds9735]);
  ...
  const subscribeKind1 = useCallback(async (kind1ID: string) => {
    ...
    return subscription;
  }, [nostrClient, resetZapList, onKind1Event, subscribeKind0fromKind1, subscribeKind9735fromKind1]);
