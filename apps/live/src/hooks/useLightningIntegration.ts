 ...
 */
export function useLightningIntegration(options: UseLightningIntegrationOptions = {}) {
   ...
   */
  const createLightningQRSlide = useCallback((lnurl: string) => {
   ...
   */
  const enableLightning = useCallback(async (targetEventId?: string): Promise<boolean> => {
    ...
    try {
      ...
      const success = await lightningService.current.enableLightning(eventIdToUse);
      ...
      if (success) {
        ...
        const errorMsg = lightningService.current.lastError || 'Unknown error';
        updatePaymentStatus(`Failed to enable Lightning: ${errorMsg}`, 'error');
        return false;
      }
    } catch (error) {
   ...
   */
  const disableLightning = useCallback(async (targetEventId?: string): Promise<boolean> => {
    const eventIdToUse = targetEventId || eventId;
    if (!eventIdToUse || !lightningService.current) {
      updatePaymentStatus('No event ID provided', 'error');
      return false;
    ...
    try {
      const success = await lightningService.current.disableLightning(eventIdToUse);
      ...
      if (success) {
        ...
        const errorMsg = lightningService.current.lastError || 'Unknown error';
        updatePaymentStatus(`Failed to disable Lightning: ${errorMsg}`, 'error');
        return false;
      }
    } catch (error) {
   ...
   */
  const handleLightningToggle = useCallback(async (checked: boolean, targetEventId?: string) => {
    const eventIdToUse = targetEventId || eventId;
    if (!eventIdToUse || !lightningService.current) {
      return;
    }
