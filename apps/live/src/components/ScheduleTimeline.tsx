...
export const ScheduleTimeline: React.FC<ScheduleTimelineProps> = ({
  ...
  const [panStart, setPanStart] = useState<{ x: number; time: number } | null>(
    null
  );
  const clickActionRef = useRef<{
    slotIndex: number | null;
    shouldOpen: boolean;
  }>({ slotIndex: null, shouldOpen: false });
  ...
  const getBaseTimeRange = () => {
    if (slots.length === 0) {
      ...
      return {
        min: Date.now() - defaultRange / 2,
        max: Date.now() + defaultRange / 2
      };
    }
  ...
  const updateSlot = useCallback(
    (index: number, updates: Partial<Slot>) => {
      const updated = [...slots];
  ...
  const addLiveToSlot = useCallback(
    (slotIndex: number, liveRef: string) => {
      if (!liveRef.trim()) return;
      const slot = slots[slotIndex];
      if (!slot) return;
      updateSlot(slotIndex, {
        lives: [...(slot.lives || []), { ref: liveRef.trim() }]
      });
    },
    [slots, updateSlot]
  );

  const removeLiveFromSlot = useCallback(
    (slotIndex: number, liveIndex: number) => {
      const slot = slots[slotIndex];
      if (!slot) return;
      updateSlot(slotIndex, {
        lives: (slot.lives || []).filter((_, i) => i !== liveIndex)
      });
    },
    [slots, updateSlot]
  );
  ...
  const tzOption: string | undefined =
    timeZone === 'local' ? undefined : timeZone;
  const timeFmt = useCallback(
    (ms: number, withSeconds = false) => {
  ...
  const dateShortFmt = useCallback(
    (ms: number) => {
      const opts: Intl.DateTimeFormatOptions = {
        month: 'short',
        day: 'numeric'
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (dragging.type === 'start') {
        ...
        if (newStart < endTime - 60000 && newStart >= minTime) {
          // min 1 minute
        ...
        if (newEnd > startTime + 60000 && newEnd <= maxTime) {
          // min 1 minute
          updateSlot(dragging.slotIndex, {
    ...
    const handleMouseUp = () => {
      ...
      const wasClick =
        clickActionRef.current.shouldOpen && !actuallyDraggedRef.current;
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      if (e.altKey) {
        ...
        const cursorTime =
          currentVisible.min + r * (currentVisible.max - currentVisible.min);
        ...
        const base = getBaseTimeRange();
        const panAmount =
          ((base.max - base.min) / (zoomLevel * 10)) * (e.deltaY > 0 ? 1 : -1);
        setTimeOffset(prev => prev + panAmount);
      }
    },
    [zoomLevel]
  ...
  const handleTimelineMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (
        ...
        (target.classList.contains('timeline-bg') &&
  ...
  const [editOverlayPosition, setEditOverlayPosition] = useState<{
    top?: string;
    bottom?: string;
    left?: number | string;
    right?: number | string;
  ...
  const calculateEditOverlayPosition = useCallback(() => {
    if (
      ...
      !timelineScrollRef.current ||
      !slotRefs.current.has(editingSlot)
    ) {
      return;
    }
    ...
    const slotVisibleBottom = Math.min(
    ...
    if (
      spaceBelow >= overlayHeight + marginTop + padding ||
      (spaceAbove < overlayHeight + padding &&
        spaceBelow >= marginTop + padding)
    ) {
      // Position below slot
    ...
    overlayLeft = Math.max(
      padding,
      Math.min(containerWidth - overlayWidth - padding, overlayLeft)
    );
  ...
  useEffect(() => {
    if (editingSlot !== null) {
      ...
      const timeoutId = setTimeout(() => {
        console.log(
          '📍 Calculating edit overlay position for slot:',
          editingSlot
        );
  ...
  return (
    ...
    >
      ...
      >
        <h3 style={{ margin: 0 }}>
          {editorMode === 'timeline' ? 'Schedule Timeline' : 'Schedule JSON'}
        </h3>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            onClick={() =>
              setEditorMode(editorMode === 'timeline' ? 'json' : 'timeline')
            }
            title={editorMode === 'timeline' ? 'Edit JSON' : 'View Timeline'}
            aria-label={
              editorMode === 'timeline' ? 'Edit JSON' : 'View Timeline'
            }
            style={{
              width: 36,
              height: 36,
          ...
          <>
              ...
              >
                      ...
                      {gridlines
                        ...
                        .map((grid, idx, arr) => {
                          const pos = getPosition(
                            new Date(grid.time).toISOString()
                          );
                ...
                <div style={{ marginTop: 24 }}>
                  {slots.map((slot, idx) => {
                    ...
                    const isActive =
                      now >= new Date(slot.startAt).getTime() &&
                      now < new Date(slot.endAt).getTime();
                        ...
                        >
                          ...
                          >
                            ...
                            <div
                              ...
                              title={`${timeFmt(new Date(slot.startAt).getTime())} → ${timeFmt(new Date(slot.endAt).getTime())} (${durationMinutes}m)`}
                            >
                              {timeFmt(new Date(slot.startAt).getTime())} →{' '}
                              {timeFmt(new Date(slot.endAt).getTime())} (
                              {durationMinutes}m)
                            </div>
                          </div>
                        </div>
                      ...
                      return (
                        <>
                          {overlaps.length > 0 && (
                            ...
                            >
                              ⚠️ Warning: This slot overlaps with slot
                              {overlaps.length !== 1 ? 's' : ''}{' '}
                              {overlaps.map(i => i + 1).join(', ')}
                            </div>
                          )}
                          {/* Readonly Pretalx info (if available) */}
                          {(slot.title ||
                            (slot.speakers && slot.speakers.length)) && (
                            <div
                              style={{
                                marginBottom: 12,
                                padding: 8,
                          ...
                          <div style={{ marginBottom: 12 }}>
                            <label
                              htmlFor={`slot-start-${editingSlot}`}
                            ...
                            >
                              {(slot.lives || []).map((live, liveIdx) => (
                                ...
                                >
                                  ...
                                  <button
                                    onClick={() =>
                                      removeLiveFromSlot(editingSlot, liveIdx)
                                    }
                                    style={{
                                      padding: '4px 8px',
                                      background: '#f44336',
            ...
            >
              ...
              <div style={{ marginLeft: 'auto' }}>
                💡 Alt+Wheel = Zoom • Shift+Wheel = Pan • Drag edges to resize,
                drag middle to move
              </div>
            </div>
          </>
            ...
            {scheduleJson !== undefined && onUpdateJson && (
              <>
                ...
                >
                  {onUploadSchedule && (
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 600,
                        cursor: 'pointer',
                        opacity: busy || !createdRoomId ? 0.7 : 1
                      }}
                    >
                      Upload schedule
                    </button>
                  ...
                  {onLoadProvidedSchedule && (
                    <button
                      onClick={onLoadProvidedSchedule}
                      style={{
                        fontSize: '0.9em',
                        padding: '6px 10px',
                        background: '#f3f4f6',
                    ...
                    >
                      Load provided schedule
                    </button>
                  )}
                  {createdRoomId && onOpenPretalxModal && (
                    <button
                      onClick={onOpenPretalxModal}
                      style={{
                        fontSize: '0.9em',
                        padding: '6px 10px',
                        background: '#f3f4f6',
                ...
                <p style={{ fontSize: 12, color: '#666', marginTop: 8 }}>
                  Tip: times must be UTC ISO format (e.g.,
                  "2025-10-29T21:00:00Z"). Lives accept note1/nevent1
                  references.
                </p>
              </>
            )}
