...
import { LiveEvent, User, Zap } from '@pubpay/shared-types';
...
export const useLiveEventStore = create<LiveEventState>()(
  subscribeWithSelector((set, get) => ({
    ...
    addEventToHistory: (event) => set((state) => ({
      ...
      state.zaps.forEach(zap => {
        ...
        if (existing) {
          ...
          zapperTotals.set(zap.payerPubkey, {
            user: zap.profile || { id: zap.payerPubkey, publicKey: zap.payerPubkey, name: 'Unknown' },
            total: zap.amount
          });
        }
      });
...
export const selectConnectionStatus = (state: LiveEventState) => state.connectionStatus;
