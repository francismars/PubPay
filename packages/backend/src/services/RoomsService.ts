...
export class RoomsService {
  ...
  public createRoom(
    configInput: Partial<RoomConfig> & { name: string }
  ): RoomConfig {
  ...
  public setSchedule(roomId: string, schedule: Schedule): { version: number } {
    ...
    for (let i = 0; i < schedule.slots.length; i++) {
      ...
      if (!slot.startAt || typeof slot.startAt !== 'string') {
        throw new Error(
          `Slot ${slotIndex}: startAt must be a non-empty string (UTC ISO format)`
        );
      }
      if (!slot.endAt || typeof slot.endAt !== 'string') {
        throw new Error(
          `Slot ${slotIndex}: endAt must be a non-empty string (UTC ISO format)`
        );
      }
      ...
      if (isNaN(start.getTime())) {
        throw new Error(
          `Slot ${slotIndex}: invalid startAt date format (use UTC ISO format, e.g., "2025-10-29T21:00:00Z")`
        );
      }
      if (isNaN(end.getTime())) {
        throw new Error(
          `Slot ${slotIndex}: invalid endAt date format (use UTC ISO format, e.g., "2025-10-29T21:00:00Z")`
        );
      }
      if (end <= start) {
      ...
      if (slotWithItems.items !== undefined) {
        throw new Error(
          `Slot ${slotIndex}: 'items' field is deprecated. Use 'lives' instead (e.g., "lives": [{"ref": "note1..."}])`
        );
      }
      if (!slot.lives) {
        throw new Error(
          `Slot ${slotIndex}: 'lives' field is required (must be an array)`
        );
      }
      ...
      for (let j = 0; j < slot.lives.length; j++) {
        ...
        if (!live || typeof live !== 'object') {
          throw new Error(
            `Slot ${slotIndex}, live ${liveIndex}: must be an object`
          );
        }
        if (!live.ref || typeof live.ref !== 'string') {
          throw new Error(
            `Slot ${slotIndex}, live ${liveIndex}: 'ref' is required and must be a string`
          );
        }
        if (!live.ref.startsWith('note1') && !live.ref.startsWith('nevent1')) {
          throw new Error(
            `Slot ${slotIndex}, live ${liveIndex}: 'ref' must start with 'note1' or 'nevent1' (got: "${live.ref}")`
          );
        }
      }
      ...
      validatedSlots.push({
        ...
        ...(slot.speakers &&
          Array.isArray(slot.speakers) && { speakers: slot.speakers })
      });
    }
  ...
  public getView(roomId: string, at?: string): ViewPayload {
    ...
    const flattenedItems =
      items.length > 0 ? items : room.config.defaultItems || [];
    const fallbackStart = activeSlot ? new Date(activeSlot.startAt) : now;
    const rotationIndex = this.computeRotationIndex(
      policy,
      fallbackStart,
      now,
      flattenedItems,
    ...
    let upcomingSlots:
      | Array<{ startAt: string; endAt: string; items: string[] }>
      | undefined;
    ...
    return {
      active: activeSlot
        ? { slotStart: activeSlot.startAt, slotEnd: activeSlot.endAt }
        : null,
  ...
  private computeRotationIndex(
    ...
    const ticks = Math.floor(
      (now.getTime() - slotStart.getTime()) / 1000 / intervalSec
    );
    ...
    if (policy === 'weighted') {
      const weights = (scheduleItems || items.map(() => ({ weight: 1 }))).map(
        i => i.weight || 1
      );
  ...
  private deterministicRandomIndex(
    seed: number,
    tick: number,
    length: number
  ): number {
