...
export const RoomCreatePage: React.FC = () => {
	...
	const create = useCallback(async () => {
		...
		try {
			...
			if (!json.success) throw new Error(json.error || 'Failed to create Multi LIVE');
			...
			<div id="noteLoaderContainer">
				<div id="noteLoader">
					<div className="note-loader-content">
						...
						<input value={name} onChange={e => setName(e.target.value)} placeholder="Multi LIVE name" />

						<label style={{ marginTop: 8 }}>Password (optional)</label>
						<input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password to access Multi LIVE" />

						<div className="button-container" style={{ marginTop: 12 }}>
							<button className="button" onClick={create} disabled={busy}>Create Multi LIVE</button>
