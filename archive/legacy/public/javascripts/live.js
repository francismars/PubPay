document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!nevent) {
    ...
    nevent =
      noteIdFromPath && noteIdFromPath !== 'live'
        ? noteIdFromPath
        : noteFromQuery;
  }
  ...
  function updateStyleURL() {
    ...
    const styles = {
      textColor: toHexColor(
        mainLayout.style.getPropertyValue('--text-color') ||
      ...
      qrShowWebLink:
        document.getElementById('qrShowWebLinkToggle')?.checked ?? true,
      qrShowNevent:
        document.getElementById('qrShowNeventToggle')?.checked ?? true,
  ...
  function applyStylesFromURL() {
    ...
    if (params.has('qrScreenBlend')) {
      ...
      if (qrScreenBlendToggle)
        qrScreenBlendToggle.checked = params.get('qrScreenBlend') === 'true';
    }
    if (params.has('qrMultiplyBlend')) {
      const qrMultiplyBlendToggle = document.getElementById(
        'qrMultiplyBlendToggle'
      );
      if (qrMultiplyBlendToggle)
        qrMultiplyBlendToggle.checked =
  ...
  function applyStylesFromLocalStorage() {
    ...
    if (!savedStyles) {
      ...
      if (qrShowWebLinkToggle)
        qrShowWebLinkToggle.checked = DEFAULT_STYLES.qrShowWebLink;
      if (qrShowNeventToggle)
        qrShowNeventToggle.checked = DEFAULT_STYLES.qrShowNevent;
      if (qrShowNoteToggle)
        qrShowNoteToggle.checked = DEFAULT_STYLES.qrShowNote;
      ...
      if (
        ...
        typeof window.updateQRSlideVisibility === 'function'
      ) {
    ...
    try {
      ...
      if (styles.bgColor) {
        const rgbaColor = hexToRgba(
          styles.bgColor,
          styles.opacity || DEFAULT_STYLES.opacity
        );
        mainLayout.style.backgroundColor = rgbaColor;
        const bgColorInput = document.getElementById('bgColorPicker');
      ...
      if (styles.bgImage) {
        ...
        if (bgImageUrl) {
          ...
          const matchingOption = bgImagePreset.querySelector(
            `option[value="${styles.bgImage}"]`
          );
          if (matchingOption) {
      ...
      if (styles.qrScreenBlend !== undefined) {
        ...
        if (qrScreenBlendToggle)
          qrScreenBlendToggle.checked = styles.qrScreenBlend;
      }
      if (styles.qrMultiplyBlend !== undefined) {
        ...
        if (qrMultiplyBlendToggle)
          qrMultiplyBlendToggle.checked = styles.qrMultiplyBlend;
      }

      // Update blend mode after setting toggles
      if (
        styles.qrScreenBlend !== undefined ||
        styles.qrMultiplyBlend !== undefined
      ) {
        updateBlendMode();
      }
      ...
      if (
        updateQRSlideVisibilityFunc &&
        typeof updateQRSlideVisibilityFunc === 'function'
      ) {
      ...
      if (styles.layoutInvert !== undefined) {
        const layoutInvertToggle =
          document.getElementById('layoutInvertToggle');
        if (layoutInvertToggle)
          layoutInvertToggle.checked = styles.layoutInvert;
      ...
      if (styles.hideZapperContent !== undefined) {
        ...
        if (hideZapperContentToggle)
          hideZapperContentToggle.checked = styles.hideZapperContent;
        document.body.classList.toggle(
          'hide-zapper-content',
          styles.hideZapperContent
        );
      }
      ...
      if (styles.showTopZappers !== undefined) {
        ...
        if (showTopZappersToggle)
          showTopZappersToggle.checked = styles.showTopZappers;
        document.body.classList.toggle(
          'show-top-zappers',
          styles.showTopZappers
        );
      }
      ...
      if (styles.textOpacity !== undefined) {
        ...
        if (textOpacityValue)
          textOpacityValue.textContent =
            Math.round(styles.textOpacity * 100) + '%';
      }
  ...
  function displayProfileNote(note) {
    ...
    const existingNotes = Array.from(
      zapsList.querySelectorAll('.profile-note')
    );
    const insertPosition = existingNotes.findIndex(
      n => parseInt(n.dataset.timestamp) < note.created_at
    );
  ...
  document
    ...
    .addEventListener('click', note1fromLoader);

  // Add Enter key support for the input field
  document
    .getElementById('note1LoaderInput')
    ...
    const sub = pool.subscribeMany(relays, [filter], {
      ...
      onclosed() {
        ...
        if (currentLiveEventInfo && reconnectionAttempts.event < 3) {
          ...
          setTimeout(() => {
            subscribeLiveEvent(
              currentLiveEventInfo.pubkey,
              currentLiveEventInfo.identifier,
              currentLiveEventInfo.kind
            );
  ...
  async function subscribeLiveEventZaps(pubkey, identifier) {
    ...
    const sub = pool.subscribeMany(relays, [filter], {
      ...
      oneose() {
        console.log(
          'subscribeLiveEventZaps() EOS - keeping subscription alive'
        );
        // Keep subscription alive for new zaps
      },
      onclosed() {
        console.log(
          'subscribeLiveEventZaps() Closed - attempting to reconnect'
        );
        // Attempt to reconnect after a delay
        setTimeout(() => {
  ...
  async function drawKind1(kind1) {
    ...
    if (!window.qrSwiper) {
      window.qrSwiper = new Swiper('.qr-swiper', {
        ...
        loop: true, // Enable loop for continuous cycling
        autoplay: {
          delay: 10000,
  ...
  function drawKind0(kind0) {
    ...
    let kind0name = displayName
      ? JSON.parse(kind0.content).displayName
      : display_Name
        ? JSON.parse(kind0.content).display_name
        : JSON.parse(kind0.content).name;
  ...
  function drawKinds9735(json9735List) {
    ...
    document.getElementById('zappedTotalValue').innerText =
    ...
    if (zapGridToggle && zapGridToggle.checked) {
      ...
      if (document.body.classList.contains('podium-enabled')) {
        ...
        for (let i = 0; i < Math.min(3, sortedZaps.length); i++) {
          ...
          console.log(
            `Applied podium-${i + 1} to zap with amount:`,
            zap.querySelector('.zapperAmountSats')?.textContent
          );
        }
      }
    }
  ...
  function displayLiveEvent(liveEvent) {
    ...
    const title =
      liveEvent.tags.find(tag => tag[0] === 'title')?.[1] || 'Live Event';
    const summary = liveEvent.tags.find(tag => tag[0] === 'summary')?.[1] || '';
    const status =
      liveEvent.tags.find(tag => tag[0] === 'status')?.[1] || 'unknown';
    ...
    const currentParticipants =
      liveEvent.tags.find(tag => tag[0] === 'current_participants')?.[1] || '0';
    const totalParticipants =
      liveEvent.tags.find(tag => tag[0] === 'total_participants')?.[1] || '0';
  ...
  function displayLiveChatMessage(chatMessage) {
    ...
    const insertPosition = existingMessages.findIndex(
      msg => parseInt(msg.dataset.timestamp) < chatMessage.created_at
    );
  ...
  function updateParticipantProfile(profile) {
    ...
    const participantElement = document.querySelector(
      `.participant[data-pubkey="${profile.pubkey}"]`
    );
    if (participantElement) {
      const pubkeySpan = participantElement.querySelector(
        '.participant-pubkey'
      );
      if (pubkeySpan) {
  ...
  function generateLiveEventQRCodes(liveEvent) {
    ...
    // Generate Lightning QR if enabled
    if (window.lightningEnabled && window.lightningLNURL) {
      console.log(
        'Generating Lightning QR for live event:',
        window.lightningLNURL
      );

      // Ensure Lightning slide exists
      const lightningSlide = document.getElementById('lightningQRSlide');
      if (!lightningSlide) {
        console.log('Creating Lightning slide for live event...');
        const swiperWrapper = document.querySelector(
          '.qr-swiper .swiper-wrapper'
        );
        if (swiperWrapper) {
          const newLightningSlide = document.createElement('div');
          newLightningSlide.className = 'swiper-slide lightning-qr-slide';
          newLightningSlide.id = 'lightningQRSlide';
          newLightningSlide.innerHTML = `
                    ...
                    `;
          swiperWrapper.appendChild(newLightningSlide);
        }
      }

      // Generate Lightning QR code with delay to ensure element is ready
      setTimeout(() => {
        const lightningQRElement = document.getElementById('lightningQRCode');
        if (lightningQRElement) {
          lightningQRElement.innerHTML = '';
          console.log('Generating Lightning QR with delay...');
          console.log('Element dimensions:', {
            width: lightningQRElement.offsetWidth,
            height: lightningQRElement.offsetHeight
          });

          try {
            const lightningQR = new QRious({
              element: lightningQRElement,
              value: window.lightningLNURL,
              size: qrSize // Use responsive size like other QR codes
            });
            console.log(
              'Lightning QR generated for live event:',
              lightningQRElement.innerHTML
            );
            console.log(
              'Lightning QR element children:',
              lightningQRElement.children
            );

            // Apply basic CSS to the generated QR code (let media queries handle responsive sizing)
            const qrCanvas = lightningQRElement.querySelector('canvas');
            if (qrCanvas) {
              qrCanvas.style.width = '100%';
              qrCanvas.style.height = 'auto';
              qrCanvas.style.marginBottom = '8px';
              qrCanvas.style.display = 'block';
              qrCanvas.style.marginLeft = 'auto';
              qrCanvas.style.marginRight = 'auto';
              console.log('Applied basic CSS to Lightning QR canvas');
            }
          } catch (error) {
            console.error('Error generating Lightning QR:', error);
          }
        }
      }, 100);
    }
    ...
    const lightningQRLink = document.getElementById('lightningQRLink');
    ...
    if (lightningQRLink && window.lightningLNURL) {
      lightningQRLink.href = `lightning:${window.lightningLNURL}`;
    }
    ...
    setTimeout(() => {
      ...
      // Set preview data after slide visibility is updated (with additional delay)
      setTimeout(() => {
        ...
        const preview4 = document.getElementById('qrDataPreview4');

        if (preview1) {
          const previewText = njumpUrl.slice(8, 28) + '...'; // Remove https:// prefix
          preview1.textContent = previewText;
        }

        if (preview2) {
          const previewText = 'nostr:' + naddrId.slice(0, 15) + '...';
        ...
        if (preview4 && window.lightningLNURL) {
          const previewText = window.lightningLNURL.substring(0, 20) + '...';
          preview4.textContent = previewText;
        }
      }, 100);
    }, 100);
  }
  ...
  function displayLiveEventZap(zapData) {
    ...
    const existingActivityItems = Array.from(
      activityContainer.querySelectorAll('.live-chat-message, .live-event-zap')
    );
    const activityInsertPosition = existingActivityItems.findIndex(
      item => parseInt(item.dataset.timestamp) < zapData.timestamp
    );
    ...
    if (activityInsertPosition === -1) {
      ...
      activityContainer.insertBefore(
        zapDiv,
        existingActivityItems[activityInsertPosition]
      );
    }
    ...
    if (zapsOnlyContainer) {
      ...
      const zapInsertPosition = existingZapItems.findIndex(
        item => parseInt(item.dataset.amount || 0) < zapData.amount
      );
      ...
      if (zapInsertPosition === -1) {
        ...
        zapsOnlyContainer.insertBefore(
          zapOnlyDiv,
          existingZapItems[zapInsertPosition]
        );
      }
    }
  ...
  function updateLiveEventZapTotal() {
    ...
    document.getElementById('zappedTotalCount').innerText =
...
*/
  ...
  document
    .querySelectorAll('.styleOptionsModalToggle')
  ...
  function applyPreset(presetName) {
    ...
    if (bgImagePreset) {
      if (preset.bgImage === '') {
        ...
        const matchingOption = Array.from(bgImagePreset.options).find(
          option => option.value === preset.bgImage
        );
        if (matchingOption) {
    ...
    document.getElementById('qrScreenBlendToggle').checked =
      preset.qrScreenBlend;
    document.getElementById('qrMultiplyBlendToggle').checked =
      preset.qrMultiplyBlend;
    document.getElementById('layoutInvertToggle').checked = preset.layoutInvert;
    document.getElementById('hideZapperContentToggle').checked =
      preset.hideZapperContent;
    document.getElementById('showTopZappersToggle').checked =
      preset.showTopZappers;
    ...
    document.getElementById('textOpacityValue').textContent =
      Math.round(preset.textOpacity * 100) + '%';
  ...
  function applyAllStyles() {
    ...
    const textOpacity = parseFloat(
      document.getElementById('textOpacitySlider').value
    );
    ...
    qrCodeContainers.forEach(qrCodeContainer => {
      if (qrCodeContainer) {
        qrCodeContainer.style.filter = document.getElementById('qrInvertToggle')
          .checked
          ? 'invert(1)'
          : 'none';
      }
    ...
    document.body.classList.toggle(
      'flex-direction-invert',
      document.getElementById('layoutInvertToggle').checked
    );
    document.body.classList.toggle(
      'hide-zapper-content',
      document.getElementById('hideZapperContentToggle').checked
    ...
    document.body.classList.toggle(
      'podium-enabled',
      document.getElementById('podiumToggle').checked
    );
  ...
  function setupStyleOptions() {
    ...
    // QR Code Management System - Unified and Simplified
    ...
    function updateQRSlideVisibilityImmediate(skipURLUpdate = false) {
      ...
      console.log('QR Visibility Update:', {
        showWebLink,
        showNevent,
        showNote,
        showLightning
      ...
      const webLinkSlide = document.querySelector('.swiper-slide:has(#qrCode)');
      const neventSlide = document.querySelector(
      ...
      console.log('Visibility update - Toggle states:', {
        showWebLink,
        showNevent,
        showNote,
        showLightning
      ...
      console.log(
        'Visibility update - Slides in swiper wrapper:',
        visibleSlides.length
      );
      console.log(
        'Visibility update - Slide IDs in swiper:',
        ...
        if (window.qrSwiper) {
          try {
            ...
            if (visibleSlides.length === 1) {
              ...
              if (window.qrSwiper.autoplay) {
                window.qrSwiper.autoplay.params.delay = 10000;
                if (window.qrSwiper.autoplay.start) {
                  window.qrSwiper.autoplay.start();
                }
              }
            }
          } catch (error) {
            console.log('Swiper update failed, will reinitialize:', error);
    ...
    function initializeQRSwiper() {
      ...
      // Use the same visibility logic as updateQRSlideVisibilityImmediate
      const webLinkToggle = document.getElementById('qrShowWebLinkToggle');
      const neventToggle = document.getElementById('qrShowNeventToggle');
      const noteToggle = document.getElementById('qrShowNoteToggle');
      ...
      // Count visible slides based on toggle states
      const visibleSlides = [];
      if (showWebLink && webLinkSlide) visibleSlides.push(webLinkSlide);
      if (showNevent && neventSlide) visibleSlides.push(neventSlide);
      if (showNote && noteSlide) visibleSlides.push(noteSlide);
      if (showLightning && lightningSlide) visibleSlides.push(lightningSlide);

      console.log('Swiper initialization - Toggle states:', {
        showWebLink,
        showNevent,
        showNote,
        showLightning
      });
      console.log('Swiper initialization - Found slides:', {
      ...
      console.log(
        'Swiper initialization - Visible slides count:',
        visibleSlides.length
      );

      if (visibleSlides.length === 0) return;

      // Create a temporary container for hidden slides
      let hiddenSlidesContainer = document.getElementById(
        'hiddenSlidesContainer'
      );
      if (!hiddenSlidesContainer) {
      ...
      // Move hidden slides out of swiper wrapper temporarily
      const allSlides = [
        webLinkSlide,
        neventSlide,
        noteSlide,
      ...
      allSlides.forEach(slide => {
        if (!visibleSlides.includes(slide)) {
          ...
          // Ensure it's in the swiper wrapper
      ...
      window.qrSwiper = new Swiper('.qr-swiper', {
        ...
        on: {
          init() {
            ...
            setTimeout(() => {
              const activeBullet = this.el.querySelector(
      ...
      function startProgressTracking() {
        ...
        const activeBullet = window.qrSwiper.el.querySelector(
          '.swiper-pagination-bullet-active'
        );
    ...
    function regenerateQRCodes() {
      ...
      const neventId = NostrTools.nip19.neventEncode({
        id: noteId,
        relays: []
      });
      const note1Id = NostrTools.nip19.noteEncode(noteId);
      ...
      if (window.lightningEnabled && window.lightningLNURL) {
        ...
        const lightningSlide = document.getElementById('lightningQRSlide');
        if (!lightningSlide) {
          console.log('Creating Lightning slide...');
          const swiperWrapper = document.querySelector(
            '.qr-swiper .swiper-wrapper'
          );
          if (swiperWrapper) {
      ...
      qrConfigs.forEach(({ id, value, linkId, previewId }) => {
        ...
        if (element) {
          ...
          try {
            console.log(`About to generate QR for ${id} with QRious...`);
            console.log(`QRious available:`, typeof QRious);
            console.log(`Element before QR generation:`, element);
            console.log(`Element dimensions:`, {
              width: element.offsetWidth,
              height: element.offsetHeight
            });

            // Use responsive size for all QR codes
            const size = qrSize;
            ...
            console.log(`QRious object created:`, qr);
            console.log(`Generated QR for ${id}:`, element.innerHTML);
            console.log(`QR element after generation:`, element);
            console.log(`QR element children:`, element.children);

            // Check if QR was actually created
            if (element.children.length === 0) {
              console.log(`QR not created, trying fallback for ${id}...`);
              // Fallback: Create canvas manually
              const canvas = document.createElement('canvas');
              const size = qrSize; // Use responsive size for all QR codes
              canvas.width = size;
              canvas.height = size;
              ...
              const fallbackQR = new QRious({
                element: canvas,
                size: size,
                value: value
              });
              console.log(`Fallback QR created for ${id}:`, canvas);
            }

            // Apply basic CSS for QR codes (let media queries handle responsive sizing)
            ...
            if (qrCanvas) {
              ...
              qrCanvas.style.marginLeft = 'auto';
              qrCanvas.style.marginRight = 'auto';
              console.log('Applied basic CSS to QR canvas');
            }
          } catch (error) {
    ...
    // Add event listeners with retry mechanism for DOM readiness
    function attachQRToggleListeners() {
      const webLinkToggle = document.getElementById('qrShowWebLinkToggle');
      const neventToggle = document.getElementById('qrShowNeventToggle');
      const noteToggle = document.getElementById('qrShowNoteToggle');

      if (webLinkToggle) {
        webLinkToggle.addEventListener('change', function (e) {
          updateQRSlideVisibility(); // This will call updateStyleURL() to save state
        });
      }
      if (neventToggle) {
        neventToggle.addEventListener('change', function (e) {
          updateQRSlideVisibility(); // This will call updateStyleURL() to save state
        });
      }
      if (noteToggle) {
        noteToggle.addEventListener('change', function (e) {
          updateQRSlideVisibility(); // This will call updateStyleURL() to save state
        });
      }
    }

    // Try to attach listeners immediately, and retry if elements don't exist yet
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', attachQRToggleListeners);
    } else {
      attachQRToggleListeners();
    }
...
async function enableLightningPayments() {
  ...
  try {
    ...
    if (data.success) {
      ...
      // Regenerate QR codes to include Lightning QR
      if (window.regenerateQRCodes) {
        window.regenerateQRCodes();
      }
      ...
      if (data.existing) {
        statusDiv.innerHTML =
          '<div class="status-info">📱 Lightning enabled (reusing existing link)</div>';
      } else {
        statusDiv.innerHTML =
...
function createLightningQRDisplay(lnurl) {
  ...
  // Generate QR code
  const qrSize = 200;
  console.log('Generating QR for popup:', {
    lnurl,
    qrSize,
...
function addLightningQRToSwiper(lnurl) {
  ...
  // Ensure the lightning slide exists in the HTML
  ...
  // Create lightning slide if it doesn't exist
  if (!lightningSlide) {
    lightningSlide = document.createElement('div');
    lightningSlide.className = 'swiper-slide lightning-qr-slide';
...
function generateLightningQR(lnurl, lightningSlide, qrContainer) {
  console.log('Generating Lightning QR:', lnurl);

  // Use the unified addLightningQRToSwiper function
  addLightningQRToSwiper(lnurl);
}
...
function reinitializeSwiper() {
  // Use the new unified swiper initialization
  initializeQRSwiper();
}
...
function organizeZapsHierarchically() {
  ...
  if (document.body.classList.contains('podium-enabled')) {
    ...
    for (let i = 0; i < Math.min(3, sortedZaps.length); i++) {
      ...
      console.log(
        `Applied podium-global-${i + 1} to zap with amount:`,
        zap.querySelector('.zapperAmountSats')?.textContent
      );
    }
  }
