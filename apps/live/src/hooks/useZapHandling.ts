...
export function useZapHandling(options: UseZapHandlingOptions = {}) {
  ...
  const getDisplayName = useCallback((profile: any): string => {
    ...
    if (typeof profile === 'string') {
      try {
        ...
        return parsed.display_name || parsed.displayName || parsed.name || 'Anonymous';
      } catch {
        return 'Anonymous';
      }
    }
  ...
  const addZapToTotals = useCallback((
    ...
    if (zapperTotals.has(pubkey)) {
      ...
      if (profile) {
        ...
        existing.picture = sanitizeImageUrl(profile.picture) || genericUserIcon;
      }
    } else {
  ...
  const calculateTopZappersFromZaps = useCallback((
    ...
    if (areKind9735Events && profiles) {
      ...
      const processedZaps = processZaps(zaps as Kind9735Event[], profiles, genericUserIcon);

      // Group by pubkey and sum amounts
      const zapperTotals = new Map<string, { amount: number; zap: ProcessedZap }>();
      ...
      for (const zap of zaps) {
        ...
        const profile = zap.kind0Profile || (profiles ? profiles.get(pubkey) : null);
        ...
        if (zapperTotals.has(pubkey)) {
          ...
          const zapperData = {
            ...
            picture:
              sanitizeImageUrl(profile?.picture || zap.picture) || genericUserIcon
          };
   ...
   */
  const processLiveEventZap = useCallback(async (
