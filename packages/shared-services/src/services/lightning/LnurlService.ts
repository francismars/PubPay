...
export class LnurlService {
  ...
  static async discoverLNURLPay(
    ...
    try {
      ...
      if (fetchError instanceof Error && fetchError.name === 'AbortError') {
        throw new Error(
          'Request timed out. The LNURL endpoint may be unreachable or slow to respond.'
        );
      }
      throw new Error(
        'Failed to connect to LNURL endpoint. Check your internet connection.'
      );
    }
    ...
    if (!response.ok) {
      if (response.status === 404) {
        ...
        throw new Error(
          'Server error. The LNURL endpoint may be experiencing issues.'
        );
      } else {
    ...
    if (!lnurlInfo.callback) {
      throw new Error(
        'This LNURL does not support payments (no callback URL found)'
      );
    }
  ...
  static async fetchInvoice(
    ...
    if (!url) {
      throw new Error(
        'Invalid LNURL format. Must be a bech32 encoded lnurl1... string.'
      );
    }
    ...
    if (description && description.trim()) {
      ...
      if (
        ...
        description.trim().length > lnurlInfo.commentAllowed
      ) {
        throw new Error(
          `Description too long. Maximum: ${lnurlInfo.commentAllowed} characters`
        );
      }
