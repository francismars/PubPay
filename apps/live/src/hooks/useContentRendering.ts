...
import { NostrClient, LiveEventService } from '@pubpay/shared-services';
import {
  Kind0Event,
  Kind1Event,
  Kind30311Event,
...
export const useContentRendering = (options: UseContentRenderingOptions) => {
  ...
  const getMentionUserName = useCallback(
            ...
            profile = await new Promise<Kind0Event | null>(resolve => {
              ...
              const subscription = nostrClient.subscribeToProfiles(
                ...
                (event: NostrEvent) => {
                  ...
                  if (
                    !existing ||
                    kind0Event.created_at > existing.created_at
                  ) {
   ...
   */
  const processNoteContent = useCallback(
    async (content: string): Promise<string> => {
      ...
      const bareNoteMatches = Array.from(
        processed.matchAll(
          /\b((note|nevent|naddr)1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]{58,})\b/gi
        )
      );
      ...
      for (const range of processedRanges) {
        processed =
          processed.substring(0, range.start) +
          range.replacement +
          processed.substring(range.end);
      }

      // Handle nostr:npub/nprofile mentions
      const nostrNpubMatches = processed.match(
      ...
      const atNpubMatches = processed.match(
  ...
  const setupLiveEventTwoColumnLayout = useCallback(() => {
    ...
    const existingZapsHeader = zapsContainer.querySelector(
      '.zaps-header'
    ) as HTMLElement | null;
   ...
   */
  const drawKind0 = useCallback(
    (kind0: Kind0Event) => {
      try {
        const profile = JSON.parse(kind0.content) as Record<string, unknown>;
        setAuthorName(
          (profile.name || profile.display_name || 'Anonymous') as string
        );
        setAuthorImage(
          sanitizeImageUrl((profile.picture as string) || '') ||
  ...
  const updateProfile = useCallback((profile: Kind0Event) => {
    ...
    try {
      profileData = JSON.parse(profile.content || '{}') as Record<
        string,
        unknown
      >;
    } catch (error) {
    ...
    const picture =
      sanitizeImageUrl((profileData.picture as string) || '') ||
      '/live/images/gradient_color.gif';
   ...
   */
  const updateLiveEventHostProfile = useCallback(
    (profile: Kind0Event) => {
      let profileData: Record<string, unknown> = {};
      try {
        profileData = JSON.parse(profile.content || '{}') as Record<
      ...
      const authorImg = getElementById<HTMLImageElement>(
   ...
   */
  const displayLiveEvent = useCallback(
    (liveEvent: Kind30311Event) => {
      logger.info('Displaying live event', ErrorCategory.RENDERING, {
      ...
      if (
        ...
        logger.info(
          'Live event already displayed, skipping',
      ...
      logger.info('Streaming URL found', ErrorCategory.VIDEO, {
        streamingUrl: streaming
      });

      // Format timestamps using service (inline for template strings)
      const formatTime =
        liveEventService.formatTimestamp.bind(liveEventService);
                    ...
                    <div id="live-video-player" class="video-player-container">
                        ...
                        <div class="video-error" id="video-error" style="display: none;">
                            ...
                            ${(() => {
                              ...
                              return sanitized
                                ? `<a href="${sanitized}" target="_blank" class="streaming-link" rel="noopener noreferrer">
                                📺 Watch in External Player
                            </a>`
                                : '<span>Streaming URL unavailable</span>';
                            })()}
                  </div>
                    </div>
                </div>
                  ...
                  recording
                    ...
                    <div class="live-event-actions">
                        ${(() => {
                          ...
                          return sanitized
                            ? `<a href="${sanitized}" target="_blank" class="recording-link" rel="noopener noreferrer">
                            🎥 Watch Recording
                        </a>`
                            : '<span>Recording URL unavailable</span>';
                        })()}
              </div>
                `
                    : ''
      ...
      setTimeout(() => {
        ...
        const qrShowNeventToggle =
          getElementById<HTMLInputElement>('qrShowNeventToggle');
        if (qrShowNeventToggle && !qrShowNeventToggle.checked) {
          // Check if any QR toggle is enabled
          const qrShowWebLinkToggle = getElementById<HTMLInputElement>(
            'qrShowWebLinkToggle'
          );
          const qrShowNoteToggle =
            getElementById<HTMLInputElement>('qrShowNoteToggle');
          const hasAnyEnabled =
   ...
   */
  const displayLiveChatMessage = useCallback(
    (chatMessage: NostrEvent) => {
      ...
      const targetContainer = getElementById('activity-list') || zapsContainer;
