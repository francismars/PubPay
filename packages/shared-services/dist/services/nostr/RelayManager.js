...
export class RelayManager {
    constructor(defaultRelays = RELAYS) {
     ...
     */
    updateConnectionStatus(relayUrl, status, error) {
     ...
     */
    updateLatency(relayUrl, latency) {
        const connection = this.connections.get(relayUrl);
        if (connection) {
     ...
     */
    getHealthyRelays() {
        return Array.from(this.connections.keys()).filter(relayUrl => this.isRelayHealthy(relayUrl));
    }
    /**
     ...
     */
    async testAllRelays() {
        const promises = Array.from(this.connections.keys()).map(relayUrl => this.testRelay(relayUrl));
        await Promise.allSettled(promises);
    }
    /**
     * Get recommended relays based on performance
     */
    getRecommendedRelays(limit = 3) {
